...
// Map the SF Symbol the app sends for a hardware output to a matching emoji,
// shown inline in the dropdown (same treatment as a mix's emoji).
...
function fillOutputDropdown(selectEl, outputs, key, { allowNone = false } = {}) {
  ...
  for (const o of outputs) {
    const opt = document.createElement("option");
    opt.value = o.uid;
    opt.textContent = outputEmoji(o.icon) + " " + o.name + (o.active ? " ●" : "");
