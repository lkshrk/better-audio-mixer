...
/// Elgato WebSocket protocol over `URLSessionWebSocketTask` (there is no official Swift SDK).
@MainActor
final class ElgatoConnection: ElgatoCommandSink {
    ...
    /// `image` is a full data URI, or nil to fall back to the manifest icon.
    func setImage(_ image: String?, context: String) {
    ...
    /// The completion runs off-actor, so only the Sendable String crosses back to the main actor.
    private func receive() {
        task?.receive { [weak self] result in
            ...
            case .failure(let error):
                // Stream Deck relaunches the plugin; lingering without a socket would leave dead keys.
                Log.error("WS receive: \(error.localizedDescription); exiting")
                exit(0)
            }
            Task { @MainActor in
