...
// N-device slot scheme.
// PlugIn=1, Box=2, devices start at kObjectID_FirstDevice.
// Each slot occupies kObjectsPerDevice consecutive IDs:
//   +0 device  +1 streamIn  +2 volIn  +3 muteIn
//   +4 streamOut  +5 volOut  +6 muteOut  +7 clock
#define kBAM_MaxDevices         8
#define kObjectsPerDevice       8
#define kObjectID_FirstDevice   16
...
// Legacy single-slot aliases used by helpers that predate the N-device refactor.
// These map to slot 0 sub-objects via the arithmetic scheme.
#define kObjectID_Device               BAM_DeviceID(0)
#define kObjectID_Stream_Input         BAM_StreamInputID(0)
#define kObjectID_Volume_Input_Master  BAM_VolumeInputID(0)
#define kObjectID_Mute_Input_Master    BAM_MuteInputID(0)
#define kObjectID_Stream_Output        BAM_StreamOutputID(0)
#define kObjectID_Volume_Output_Master BAM_VolumeOutputID(0)
#define kObjectID_Mute_Output_Master   BAM_MuteOutputID(0)
#define kObjectID_Pitch_Adjust         (BAM_DeviceID(0) + 6)  // reuse muteOut slot offset — pitch kept on slot 0 only
#define kObjectID_ClockSource          BAM_ClockID(0)
#define kObjectID_Device2              BAM_DeviceID(1)

// Arithmetic helpers — inline so no runtime cost.
static inline AudioObjectID BAM_DeviceID(int i)       { return (AudioObjectID)(kObjectID_FirstDevice + i * kObjectsPerDevice); }
static inline AudioObjectID BAM_StreamInputID(int i)  { return BAM_DeviceID(i) + 1; }
static inline AudioObjectID BAM_VolumeInputID(int i)  { return BAM_DeviceID(i) + 2; }
static inline AudioObjectID BAM_MuteInputID(int i)    { return BAM_DeviceID(i) + 3; }
static inline AudioObjectID BAM_StreamOutputID(int i) { return BAM_DeviceID(i) + 4; }
static inline AudioObjectID BAM_VolumeOutputID(int i) { return BAM_DeviceID(i) + 5; }
static inline AudioObjectID BAM_MuteOutputID(int i)   { return BAM_DeviceID(i) + 6; }
static inline AudioObjectID BAM_ClockID(int i)        { return BAM_DeviceID(i) + 7; }

// Returns slot index [0..kBAM_MaxDevices-1] for any sub-object of a device slot, or -1.
static inline int BAM_DeviceIndexForObject(AudioObjectID oid)
{
    if (oid < kObjectID_FirstDevice) return -1;
    int slot = (int)(oid - kObjectID_FirstDevice) / kObjectsPerDevice;
    if (slot >= kBAM_MaxDevices) return -1;
    return slot;
}

// True if oid is the device object itself (offset 0 within a slot).
static inline Boolean BAM_IsDeviceObject(AudioObjectID oid)
{
    if (oid < kObjectID_FirstDevice) return false;
    int offset = (int)(oid - kObjectID_FirstDevice) % kObjectsPerDevice;
    return (offset == 0) && (BAM_DeviceIndexForObject(oid) >= 0);
}

// True if oid is a stream (offsets 1 or 4).
static inline Boolean BAM_IsStreamObject(AudioObjectID oid)
{
    if (oid < kObjectID_FirstDevice) return false;
    int offset = (int)(oid - kObjectID_FirstDevice) % kObjectsPerDevice;
    return (offset == 1 || offset == 4) && (BAM_DeviceIndexForObject(oid) >= 0);
}

// True if oid is a control (offsets 2,3,5,6,7).
static inline Boolean BAM_IsControlObject(AudioObjectID oid)
{
    if (oid < kObjectID_FirstDevice) return false;
    int offset = (int)(oid - kObjectID_FirstDevice) % kObjectsPerDevice;
    return (offset >= 2 && offset <= 7) && (BAM_DeviceIndexForObject(oid) >= 0);
}

// True if oid is the INPUT stream of its slot (offset 1). Output stream = offset 4.
static inline Boolean BAM_IsInputStream(AudioObjectID oid)
{
    if (oid < kObjectID_FirstDevice) return false;
    return ((int)(oid - kObjectID_FirstDevice) % kObjectsPerDevice) == 1;
}
...
// BAM custom properties selectors.
#define kBAMProperty_Claimed    'bmcl'   // device-scope: UInt32 via CFPropertyList
#define kBAMProperty_MixName    'bmnm'   // device-scope: CFString
#define kBAMProperty_Channels   'bmch'   // device-scope: UInt32 via CFPropertyList
// Plugin-scope config written by controller to claim/name/channel all slots atomically.
// Value: CFArray of CFDictionary{slot:CFNumber, claimed:CFNumber, name:CFString, channels:CFNumber}
#define kBAMProperty_MixConfig  'bmcf'
#define kBAM_NumberDeviceCustomProperties  3
#define kBAM_NumberPlugInCustomProperties  1
...
// Per-slot state arrays indexed [0..kBAM_MaxDevices-1].
static pthread_mutex_t              gDevice_IOMutex                     = PTHREAD_MUTEX_INITIALIZER;
static const UInt32                 kDevice_RingBufferSize              = 16384;

static Float64      gBAM_SampleRate[kBAM_MaxDevices];
static Float64      gBAM_RequestedSampleRate[kBAM_MaxDevices];
static UInt64       gBAM_IOIsRunning[kBAM_MaxDevices];
static Float32*     gBAM_RingBuffer[kBAM_MaxDevices];
static Float64      gBAM_HostTicksPerFrame[kBAM_MaxDevices];
static Float64      gBAM_AdjustedTicksPerFrame[kBAM_MaxDevices];
static Float64      gBAM_PreviousTicks[kBAM_MaxDevices];
static UInt64       gBAM_NumberTimeStamps[kBAM_MaxDevices];
static Float64      gBAM_AnchorSampleTime[kBAM_MaxDevices];
static UInt64       gBAM_AnchorHostTime[kBAM_MaxDevices];

static bool         gBAM_StreamInputActive[kBAM_MaxDevices];
static bool         gBAM_StreamOutputActive[kBAM_MaxDevices];

static Float32      gBAM_Volume[kBAM_MaxDevices];
static bool         gBAM_Mute[kBAM_MaxDevices];
static Float32      gBAM_PitchAdjust[kBAM_MaxDevices];
static bool         gBAM_PitchEnabled[kBAM_MaxDevices];
static UInt32       gBAM_ClockSource[kBAM_MaxDevices];

// Claim / identity per slot.
static UInt32       gBAM_Claimed[kBAM_MaxDevices];
static CFStringRef  gBAM_MixName[kBAM_MaxDevices];
static UInt32       gBAM_Channels[kBAM_MaxDevices];
...
// Per-slot ObjectInfo tables built at runtime from slot index.
// Each slot has: streamIn, volIn, muteIn, streamOut, volOut, muteOut, clock (7 sub-objects).
// Slot 0 also keeps pitch adjust at offset 6 (muteOut). Per-spec pitch kept on slot 0 only.
#define kSlot_SubObjectCount  7  // sub-objects per slot (not counting the device itself)

static struct ObjectInfo BAM_SlotObjectList(int slot, UInt32 index)
{
    // Returns the index-th ObjectInfo for the given slot.
    struct ObjectInfo tbl[7] = {
        { BAM_StreamInputID(slot),  kObjectType_Stream,  kAudioObjectPropertyScopeInput  },
        { BAM_VolumeInputID(slot),  kObjectType_Control, kAudioObjectPropertyScopeInput  },
        { BAM_MuteInputID(slot),    kObjectType_Control, kAudioObjectPropertyScopeInput  },
        { BAM_StreamOutputID(slot), kObjectType_Stream,  kAudioObjectPropertyScopeOutput },
        { BAM_VolumeOutputID(slot), kObjectType_Control, kAudioObjectPropertyScopeOutput },
        { BAM_MuteOutputID(slot),   kObjectType_Control, kAudioObjectPropertyScopeOutput },
        { BAM_ClockID(slot),        kObjectType_Control, kAudioObjectPropertyScopeGlobal },
    };
    return tbl[index];
}

// Count of sub-objects for a slot matching a given scope (or all if ScopeGlobal).
static UInt32 BAM_SlotObjectCount(int slot, AudioObjectPropertyScope scope)
{
    (void)slot;
    if (scope == kAudioObjectPropertyScopeGlobal) return kSlot_SubObjectCount;
    UInt32 n = 0;
    for (UInt32 i = 0; i < kSlot_SubObjectCount; i++) {
        struct ObjectInfo oi = BAM_SlotObjectList(slot, i);
        if (oi.scope == scope) n++;
    }
    return n;
}

static UInt32 BAM_SlotStreamCount(int slot, AudioObjectPropertyScope scope)
{
    (void)slot;
    UInt32 n = 0;
    for (UInt32 i = 0; i < kSlot_SubObjectCount; i++) {
        struct ObjectInfo oi = BAM_SlotObjectList(slot, i);
        if (oi.type == kObjectType_Stream && (oi.scope == scope || scope == kAudioObjectPropertyScopeGlobal)) n++;
    }
    return n;
}

static UInt32 BAM_SlotControlCount(int slot, AudioObjectPropertyScope scope)
{
    (void)slot;
    UInt32 n = 0;
    for (UInt32 i = 0; i < kSlot_SubObjectCount; i++) {
        struct ObjectInfo oi = BAM_SlotObjectList(slot, i);
        if (oi.type == kObjectType_Control && (oi.scope == scope || scope == kAudioObjectPropertyScopeGlobal)) n++;
    }
    return n;
}

// Initialize per-slot defaults. Called once from BlackHole_Initialize.
static void BAM_InitSlots(void)
{
    for (int i = 0; i < kBAM_MaxDevices; i++) {
        gBAM_SampleRate[i]          = 48000.0;
        gBAM_RequestedSampleRate[i] = 0.0;
        gBAM_IOIsRunning[i]         = 0;
        gBAM_RingBuffer[i]          = NULL;
        gBAM_HostTicksPerFrame[i]   = 0.0;
        gBAM_AdjustedTicksPerFrame[i] = 0.0;
        gBAM_PreviousTicks[i]       = 0.0;
        gBAM_NumberTimeStamps[i]    = 0;
        gBAM_AnchorSampleTime[i]    = 0.0;
        gBAM_AnchorHostTime[i]      = 0;
        gBAM_StreamInputActive[i]   = true;
        gBAM_StreamOutputActive[i]  = true;
        gBAM_Volume[i]              = 1.0;
        gBAM_Mute[i]                = false;
        gBAM_PitchAdjust[i]         = 0.5;
        gBAM_PitchEnabled[i]        = false;
        gBAM_ClockSource[i]         = 0;
        gBAM_Claimed[i]             = 0;
        gBAM_MixName[i]             = NULL;
        gBAM_Channels[i]            = kNumber_Of_Channels;
    }
}

// Legacy size helpers — now delegate to per-slot helpers.
static UInt32 device_object_list_size(AudioObjectPropertyScope scope, AudioObjectID objectID)
{
    int slot = BAM_DeviceIndexForObject(objectID);
    if (slot < 0) return 0;
    return BAM_SlotObjectCount(slot, scope);
}
...
static UInt32 device_stream_list_size(AudioObjectPropertyScope scope, AudioObjectID objectID)
{
    int slot = BAM_DeviceIndexForObject(objectID);
    if (slot < 0) return 0;
    return BAM_SlotStreamCount(slot, scope);
}
...
static UInt32 device_control_list_size(AudioObjectPropertyScope scope, AudioObjectID objectID)
{
    int slot = BAM_DeviceIndexForObject(objectID);
    if (slot < 0) return 0;
    return BAM_SlotControlCount(slot, scope);
}
...
static OSStatus	BlackHole_Initialize(AudioServerPlugInDriverRef inDriver, AudioServerPlugInHostRef inHost)
{
	...
	//	initialize per-slot state arrays
	BAM_InitSlots();
...
static OSStatus	BlackHole_AddDeviceClient(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, const AudioServerPlugInClientInfo* inClientInfo)
{
	...
	FailWithAction(!BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_AddDeviceClient: bad device ID");
...
static OSStatus	BlackHole_RemoveDeviceClient(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, const AudioServerPlugInClientInfo* inClientInfo)
{
	...
	FailWithAction(!BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_RemoveDeviceClient: bad device ID");
...
static OSStatus	BlackHole_PerformDeviceConfigurationChange(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt64 inChangeAction, void* inChangeInfo)
{
	...
	int _cfgSlot = BAM_DeviceIndexForObject(inDeviceObjectID);
	FailWithAction(_cfgSlot < 0 || !BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_PerformDeviceConfigurationChange: bad device ID");
    switch(inChangeAction)
    {
        case ChangeAction_EnablePitchControl:
            pthread_mutex_lock(&gPlugIn_StateMutex);
            gBAM_PitchEnabled[_cfgSlot] = true;
            pthread_mutex_unlock(&gPlugIn_StateMutex);
            break;
        case ChangeAction_DisablePitchControl:
            pthread_mutex_lock(&gPlugIn_StateMutex);
            gBAM_PitchEnabled[_cfgSlot] = false;
            pthread_mutex_unlock(&gPlugIn_StateMutex);
            break;
        case ChangeAction_SetSampleRate:
            pthread_mutex_lock(&gPlugIn_StateMutex);
            newSampleRate = gBAM_RequestedSampleRate[_cfgSlot];
            pthread_mutex_unlock(&gPlugIn_StateMutex);
            ...
            pthread_mutex_lock(&gPlugIn_StateMutex);
            gBAM_SampleRate[_cfgSlot] = newSampleRate;
            struct mach_timebase_info theTimeBaseInfo;
            mach_timebase_info(&theTimeBaseInfo);
            Float64 theHostClockFrequency = (Float64)theTimeBaseInfo.denom / (Float64)theTimeBaseInfo.numer;
            theHostClockFrequency *= 1000000000.0;
            gBAM_HostTicksPerFrame[_cfgSlot] = theHostClockFrequency / gBAM_SampleRate[_cfgSlot];
            gBAM_AdjustedTicksPerFrame[_cfgSlot] = gBAM_HostTicksPerFrame[_cfgSlot] - gBAM_HostTicksPerFrame[_cfgSlot]/100.0 * 2.0*(gBAM_PitchAdjust[_cfgSlot] - 0.5);
            pthread_mutex_unlock(&gPlugIn_StateMutex);
            break;
    };
...
static OSStatus	BlackHole_AbortDeviceConfigurationChange(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt64 inChangeAction, void* inChangeInfo)
{
	...
	//	check the arguments
	FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_AbortDeviceConfigurationChange: bad driver reference");
	FailWithAction(!BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_AbortDeviceConfigurationChange: bad device ID");
...
static Boolean	BlackHole_HasProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress)
{
	...
	//	Route by object type using arithmetic helpers (IDs are not constants, so no switch).
...
static OSStatus	BlackHole_GetPlugInPropertyDataSize(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32* outDataSize)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyOwnedObjects:
			{
				// Box always owned; devices only when claimed and box acquired.
				UInt32 _ownedCount = 1; // Box
				if(gBox_Acquired)
				{
					for(int _oci = 0; _oci < kBAM_MaxDevices; ++_oci)
					{
						if(gBAM_Claimed[_oci]) ++_ownedCount;
					}
				}
				*outDataSize = _ownedCount * sizeof(AudioObjectID);
			}
			break;
		...
		case kAudioPlugInPropertyDeviceList:
			{
				UInt32 _claimedCount = 0;
				if(gBox_Acquired)
				{
					for(int _ci = 0; _ci < kBAM_MaxDevices; ++_ci)
					{
						if(gBAM_Claimed[_ci]) ++_claimedCount;
					}
				}
				*outDataSize = _claimedCount * sizeof(AudioObjectID);
			}
			break;
		...
		case kAudioObjectPropertyCustomPropertyInfoList:
			*outDataSize = kBAM_NumberPlugInCustomProperties * sizeof(AudioServerPlugInCustomPropertyInfo);
			break;
...
static OSStatus	BlackHole_GetPlugInPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyOwnedObjects:
			//	Plugin owns: Box + all claimed devices (when box acquired).
			{
				UInt32 _ooMax = inDataSize / sizeof(AudioObjectID);
				UInt32 _ooOut = 0;
				if(_ooOut < _ooMax) ((AudioObjectID*)outData)[_ooOut++] = kObjectID_Box;
				if(gBox_Acquired)
				{
					for(int _ooi = 0; _ooi < kBAM_MaxDevices && _ooOut < _ooMax; ++_ooi)
					{
						if(gBAM_Claimed[_ooi])
						{
							((AudioObjectID*)outData)[_ooOut++] = BAM_DeviceID(_ooi);
						}
					}
				}
				*outDataSize = _ooOut * sizeof(AudioObjectID);
			}
			break;
		...
		case kAudioPlugInPropertyDeviceList:
			//	Publish only claimed slots (Stage 2).
			{
				UInt32 _dlOut = 0;
				UInt32 _dlMax = inDataSize / sizeof(AudioObjectID);
				if(gBox_Acquired)
				{
					for(int _dli = 0; _dli < kBAM_MaxDevices && _dlOut < _dlMax; ++_dli)
					{
						if(gBAM_Claimed[_dli])
						{
							((AudioObjectID*)outData)[_dlOut++] = BAM_DeviceID(_dli);
						}
					}
				}
				*outDataSize = _dlOut * sizeof(AudioObjectID);
			}
			break;
		...
		case kAudioPlugInPropertyTranslateUIDToDevice:
			...
			{
				CFStringRef _qualUID = *((CFStringRef*)inQualifierData);
				AudioObjectID _foundDevice = kAudioObjectUnknown;
				for(int _ui = 0; _ui < kBAM_MaxDevices; ++_ui)
				{
					CFStringRef _slotUID = CFStringCreateWithFormat(NULL, NULL, CFSTR("BAM_UID_%d"), _ui);
					if(CFStringCompare(_qualUID, _slotUID, 0) == kCFCompareEqualTo)
					{
						_foundDevice = BAM_DeviceID(_ui);
						CFRelease(_slotUID);
						break;
					}
					CFRelease(_slotUID);
				}
				*((AudioObjectID*)outData) = _foundDevice;
				*outDataSize = sizeof(AudioObjectID);
			}
			break;
		...
		case kAudioObjectPropertyCustomPropertyInfoList:
			{
				AudioServerPlugInCustomPropertyInfo theInfos[kBAM_NumberPlugInCustomProperties] =
				{
					{ kBAMProperty_MixConfig, kAudioServerPlugInCustomPropertyDataTypeCFPropertyList, kAudioServerPlugInCustomPropertyDataTypeNone }
				};
				UInt32 theNumberToReturn = inDataSize / sizeof(AudioServerPlugInCustomPropertyInfo);
				if(theNumberToReturn > kBAM_NumberPlugInCustomProperties) { theNumberToReturn = kBAM_NumberPlugInCustomProperties; }
				memcpy(outData, theInfos, theNumberToReturn * sizeof(AudioServerPlugInCustomPropertyInfo));
				*outDataSize = theNumberToReturn * sizeof(AudioServerPlugInCustomPropertyInfo);
			}
			break;

		case kBAMProperty_MixConfig:
			FailWithAction(inDataSize < sizeof(CFPropertyListRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetPlugInPropertyData: not enough space for kBAMProperty_MixConfig");
			pthread_mutex_lock(&gPlugIn_StateMutex);
			*((CFPropertyListRef*)outData) = gBAM_MixConfig;
			if(gBAM_MixConfig != NULL) CFRetain(gBAM_MixConfig);
			pthread_mutex_unlock(&gPlugIn_StateMutex);
			*outDataSize = sizeof(CFPropertyListRef);
			break;
...
static OSStatus	BlackHole_SetPlugInPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData, UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2])
{
	...
	*outNumberPropertiesChanged = 0;
	...
	switch(inAddress->mSelector)
	{
		case kBAMProperty_MixConfig:
			{
				FailWithAction(inDataSize != sizeof(CFPropertyListRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_SetPlugInPropertyData: wrong size for kBAMProperty_MixConfig");
				CFPropertyListRef theNewConfig = *((const CFPropertyListRef*)inData);
				//	Config is an array of dicts {slot, claimed, name, channels}; parse into per-slot
				//	state, then notify the device list changed so coreaudiod republishes claimed slots.
				FailWithAction(theNewConfig == NULL || CFGetTypeID(theNewConfig) != CFArrayGetTypeID(), theAnswer = kAudioHardwareIllegalOperationError, Done, "BlackHole_SetPlugInPropertyData: MixConfig is not a CFArray");
				CFArrayRef theArray = (CFArrayRef)theNewConfig;
				pthread_mutex_lock(&gPlugIn_StateMutex);

				if(theNewConfig != gBAM_MixConfig)
				{
					CFRetain(theNewConfig);
					if(gBAM_MixConfig != NULL) CFRelease(gBAM_MixConfig);
					gBAM_MixConfig = theNewConfig;
				}

				//	Rebuild claim state from scratch: clear all, then apply entries.
				for(int _ms = 0; _ms < kBAM_MaxDevices; ++_ms)
				{
					gBAM_Claimed[_ms] = 0;
				}

				CFIndex theCount = CFArrayGetCount(theArray);
				for(CFIndex _mi = 0; _mi < theCount; ++_mi)
				{
					CFDictionaryRef theEntry = (CFDictionaryRef)CFArrayGetValueAtIndex(theArray, _mi);
					if(theEntry == NULL || CFGetTypeID(theEntry) != CFDictionaryGetTypeID()) { continue; }

					SInt32 theSlot = -1;
					CFNumberRef theSlotNum = (CFNumberRef)CFDictionaryGetValue(theEntry, CFSTR("slot"));
					if(theSlotNum == NULL || CFGetTypeID(theSlotNum) != CFNumberGetTypeID()) { continue; }
					CFNumberGetValue(theSlotNum, kCFNumberSInt32Type, &theSlot);
					if(theSlot < 0 || theSlot >= kBAM_MaxDevices) { continue; }

					SInt32 theClaimed = 1;
					CFNumberRef theClaimedNum = (CFNumberRef)CFDictionaryGetValue(theEntry, CFSTR("claimed"));
					if(theClaimedNum != NULL && CFGetTypeID(theClaimedNum) == CFNumberGetTypeID())
					{
						CFNumberGetValue(theClaimedNum, kCFNumberSInt32Type, &theClaimed);
					}
					gBAM_Claimed[theSlot] = (UInt32)(theClaimed != 0);

					CFStringRef theName = (CFStringRef)CFDictionaryGetValue(theEntry, CFSTR("name"));
					if(theName != NULL && CFGetTypeID(theName) == CFStringGetTypeID())
					{
						CFRetain(theName);
						if(gBAM_MixName[theSlot] != NULL) CFRelease(gBAM_MixName[theSlot]);
						gBAM_MixName[theSlot] = theName;
					}

					CFNumberRef theChNum = (CFNumberRef)CFDictionaryGetValue(theEntry, CFSTR("channels"));
					if(theChNum != NULL && CFGetTypeID(theChNum) == CFNumberGetTypeID())
					{
						SInt32 theCh = 2;
						CFNumberGetValue(theChNum, kCFNumberSInt32Type, &theCh);
						if(theCh > 0) gBAM_Channels[theSlot] = (UInt32)theCh;
					}
				}
				pthread_mutex_unlock(&gPlugIn_StateMutex);

				*outNumberPropertiesChanged = 2;
				outChangedAddresses[0].mSelector = kBAMProperty_MixConfig;
				outChangedAddresses[0].mScope = kAudioObjectPropertyScopeGlobal;
				outChangedAddresses[0].mElement = kAudioObjectPropertyElementMain;
				outChangedAddresses[1].mSelector = kAudioPlugInPropertyDeviceList;
...
static OSStatus	BlackHole_GetBoxPropertyDataSize(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32* outDataSize)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioBoxPropertyDeviceList:
			{
				//	Must match the PlugIn device list exactly: only claimed slots, and only
				//	while the box is acquired. Reporting phantom devices the plugin device
				//	list omits wedges coreaudiod enumeration.
				pthread_mutex_lock(&gPlugIn_StateMutex);
				UInt32 _bdlCount = 0;
				if(gBox_Acquired)
				{
					for(int _bi = 0; _bi < kBAM_MaxDevices; ++_bi)
					{
						if(gBAM_Claimed[_bi]) ++_bdlCount;
					}
				}
				*outDataSize = _bdlCount * sizeof(AudioObjectID);
				pthread_mutex_unlock(&gPlugIn_StateMutex);
			}
			break;
...
static OSStatus	BlackHole_GetBoxPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioBoxPropertyDeviceList:
			//	This is used to indicate which devices came from this box. Must match the
			//	PlugIn device list exactly: claimed slots only, box acquired only.
			pthread_mutex_lock(&gPlugIn_StateMutex);
			{
				UInt32 _bdMax = inDataSize / sizeof(AudioObjectID);
				UInt32 _bdOut = 0;
				if(gBox_Acquired)
				{
					for(int _bdi = 0; _bdi < kBAM_MaxDevices && _bdOut < _bdMax; ++_bdi)
					{
						if(gBAM_Claimed[_bdi])
						{
							((AudioObjectID*)outData)[_bdOut++] = BAM_DeviceID(_bdi);
						}
					}
				}
				*outDataSize = _bdOut * sizeof(AudioObjectID);
			}
			pthread_mutex_unlock(&gPlugIn_StateMutex);
			break;
...
static Boolean	BlackHole_HasDeviceProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress)
{
	...
	FailIf(!BAM_IsDeviceObject(inObjectID), Done, "BlackHole_HasDeviceProperty: not the device object");
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyCustomPropertyInfoList:
		case kBAMProperty_Claimed:
		case kBAMProperty_MixName:
		case kBAMProperty_Channels:
			theAnswer = true;
			break;
...
static OSStatus	BlackHole_IsDevicePropertySettable(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, Boolean* outIsSettable)
{
	...
	FailWithAction(!BAM_IsDeviceObject(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_IsDevicePropertySettable: not the device object");
...
static OSStatus	BlackHole_GetDevicePropertyDataSize(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32* outDataSize)
{
	...
	FailWithAction(!BAM_IsDeviceObject(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_GetDevicePropertyDataSize: not the device object");
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyCustomPropertyInfoList:
			*outDataSize = kBAM_NumberDeviceCustomProperties * sizeof(AudioServerPlugInCustomPropertyInfo);
			break;
...
static OSStatus	BlackHole_GetDevicePropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	FailWithAction(!BAM_IsDeviceObject(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_GetDevicePropertyData: not the device object");
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyName:
			...
			{
				int _slot = BAM_DeviceIndexForObject(inObjectID);
				if (_slot >= 0 && gBAM_MixName[_slot] != NULL) {
					*((CFStringRef*)outData) = (CFStringRef)CFRetain(gBAM_MixName[_slot]);
				} else {
					// Default name: "BAM N" (slot 0 back-compat: "BAM 0")
					*((CFStringRef*)outData) = CFStringCreateWithFormat(NULL, NULL, CFSTR("BAM %d"), _slot >= 0 ? _slot : 0);
				}
				*outDataSize = sizeof(CFStringRef);
			}
			break;
		...
		case kAudioObjectPropertyOwnedObjects:
			//	Calculate the number of items that have been requested.
			{
				...
				for (UInt32 _i = 0, _k = 0; _k < theNumberItemsToFetch && _i < kSlot_SubObjectCount; _i++)
				{
					struct ObjectInfo _oi = BAM_SlotObjectList(_slot, _i);
					if (_oi.scope == inAddress->mScope || inAddress->mScope == kAudioObjectPropertyScopeGlobal)
					{
						((AudioObjectID*)outData)[_k++] = _oi.id;
					}
				}
		...
		case kAudioDevicePropertyDeviceUID:
			//	Persistent per-slot UID: "BAM_UID_N" (slot-indexed, unique across reboots).
			...
			{
				int _slot = BAM_DeviceIndexForObject(inObjectID);
				*((CFStringRef*)outData) = CFStringCreateWithFormat(NULL, NULL, CFSTR("BAM_UID_%d"), _slot >= 0 ? _slot : 0);
				*outDataSize = sizeof(CFStringRef);
			}
			break;
        ...
        case kAudioDevicePropertyDeviceIsRunning:
            ...
            FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetDevicePropertyData: not enough space for the return value of kAudioDevicePropertyDeviceIsRunning for the device");
            {
                int _slot = BAM_DeviceIndexForObject(inObjectID);
                pthread_mutex_lock(&gPlugIn_StateMutex);
                *((UInt32*)outData) = (_slot >= 0 && gBAM_IOIsRunning[_slot] > 0) ? 1 : 0;
                pthread_mutex_unlock(&gPlugIn_StateMutex);
                *outDataSize = sizeof(UInt32);
            }
            break;
		...
		case kAudioDevicePropertyStreams:
			{
				...
				{
					struct ObjectInfo _oi = BAM_SlotObjectList(_slot, _i);
					if (_oi.type == kObjectType_Stream &&
					    (_oi.scope == inAddress->mScope || inAddress->mScope == kAudioObjectPropertyScopeGlobal))
					{
						((AudioObjectID*)outData)[_k++] = _oi.id;
					}
				}
		...
		case kAudioObjectPropertyControlList:
			{
				...
				{
					struct ObjectInfo _oi = BAM_SlotObjectList(_slot, _i);
					// Pitch adjust only on slot 0 and only when enabled.
					if (_oi.id == BAM_MuteOutputID(0) && !gBAM_PitchEnabled[0]) continue;
					if (_oi.type == kObjectType_Control)
						((AudioObjectID*)outData)[_k++] = _oi.id;
				}
				pthread_mutex_unlock(&gPlugIn_StateMutex);
				*outDataSize = theNumberItemsToFetch * sizeof(AudioObjectID);
		...
		case kAudioDevicePropertyIsHidden:
			//	All BAM slots are visible; hidden is controlled by DeviceList publication.
		...
		case kAudioObjectPropertyCustomPropertyInfoList:
			{
				//	Advertise the BAM custom properties so the Host marshals them to/from clients.
				AudioServerPlugInCustomPropertyInfo theInfos[kBAM_NumberDeviceCustomProperties] =
				{
					{ kBAMProperty_Claimed,  kAudioServerPlugInCustomPropertyDataTypeCFPropertyList, kAudioServerPlugInCustomPropertyDataTypeNone },
					{ kBAMProperty_Channels, kAudioServerPlugInCustomPropertyDataTypeCFPropertyList, kAudioServerPlugInCustomPropertyDataTypeNone },
					{ kBAMProperty_MixName,  kAudioServerPlugInCustomPropertyDataTypeCFString,       kAudioServerPlugInCustomPropertyDataTypeNone }
				};
				UInt32 theNumberToReturn = inDataSize / sizeof(AudioServerPlugInCustomPropertyInfo);
				if(theNumberToReturn > kBAM_NumberDeviceCustomProperties) { theNumberToReturn = kBAM_NumberDeviceCustomProperties; }
				memcpy(outData, theInfos, theNumberToReturn * sizeof(AudioServerPlugInCustomPropertyInfo));
				*outDataSize = theNumberToReturn * sizeof(AudioServerPlugInCustomPropertyInfo);
			}
			break;
		...
		case kBAMProperty_Claimed:
			{
				FailWithAction(inDataSize < sizeof(CFPropertyListRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetDevicePropertyData: not enough space for kBAMProperty_Claimed");
				int _slot = BAM_DeviceIndexForObject(inObjectID);
				pthread_mutex_lock(&gPlugIn_StateMutex);
				UInt32 theValue = (_slot >= 0) ? gBAM_Claimed[_slot] : 0;
				pthread_mutex_unlock(&gPlugIn_StateMutex);
				*((CFNumberRef*)outData) = CFNumberCreate(NULL, kCFNumberSInt32Type, &theValue);
				*outDataSize = sizeof(CFPropertyListRef);
			}
			break;
		...
		case kBAMProperty_Channels:
			{
				FailWithAction(inDataSize < sizeof(CFPropertyListRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetDevicePropertyData: not enough space for kBAMProperty_Channels");
				int _slot = BAM_DeviceIndexForObject(inObjectID);
				pthread_mutex_lock(&gPlugIn_StateMutex);
				UInt32 theValue = (_slot >= 0) ? gBAM_Channels[_slot] : kNumber_Of_Channels;
				pthread_mutex_unlock(&gPlugIn_StateMutex);
				*((CFNumberRef*)outData) = CFNumberCreate(NULL, kCFNumberSInt32Type, &theValue);
				*outDataSize = sizeof(CFPropertyListRef);
			}
		...
		case kBAMProperty_MixName:
			{
				FailWithAction(inDataSize < sizeof(CFStringRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetDevicePropertyData: not enough space for kBAMProperty_MixName");
				int _slot = BAM_DeviceIndexForObject(inObjectID);
				pthread_mutex_lock(&gPlugIn_StateMutex);
				CFStringRef _mn = (_slot >= 0) ? gBAM_MixName[_slot] : NULL;
				*((CFStringRef*)outData) = _mn ? (CFStringRef)CFRetain(_mn) : CFSTR("");
				pthread_mutex_unlock(&gPlugIn_StateMutex);
				*outDataSize = sizeof(CFStringRef);
			}
			break;
...
static OSStatus	BlackHole_SetDevicePropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData, UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2])
{
	...
	FailWithAction(!BAM_IsDeviceObject(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_SetDevicePropertyData: not a device object");
	...
	*outNumberPropertiesChanged = 0;
	...
	switch(inAddress->mSelector)
	{
		case kAudioDevicePropertyNominalSampleRate:
			...
			int _srSlot = BAM_DeviceIndexForObject(inObjectID);
			pthread_mutex_lock(&gPlugIn_StateMutex);
			theOldSampleRate = gBAM_SampleRate[_srSlot];
			gBAM_RequestedSampleRate[_srSlot] = *((const Float64*)inData);
			pthread_mutex_unlock(&gPlugIn_StateMutex);
			if(*((const Float64*)inData) != theOldSampleRate)
			{
				AudioObjectID _srDevice = BAM_DeviceID(_srSlot);
				dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{ gPlugIn_Host->RequestDeviceConfigurationChange(gPlugIn_Host, _srDevice, ChangeAction_SetSampleRate, NULL); });
			}
			break;

		case kBAMProperty_Claimed:
			{
				FailWithAction(inDataSize != sizeof(CFPropertyListRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_SetDevicePropertyData: wrong size for kBAMProperty_Claimed");
				CFNumberRef theNumber = *((const CFNumberRef*)inData);
				FailWithAction(theNumber == NULL || CFGetTypeID(theNumber) != CFNumberGetTypeID(), theAnswer = kAudioHardwareIllegalOperationError, Done, "BlackHole_SetDevicePropertyData: kBAMProperty_Claimed not a CFNumber");
				UInt32 theValue = 0;
				CFNumberGetValue(theNumber, kCFNumberSInt32Type, &theValue);
				int _clSlot = BAM_DeviceIndexForObject(inObjectID);
				if(_clSlot >= 0)
				{
					pthread_mutex_lock(&gPlugIn_StateMutex);
					UInt32 _oldClaimed = gBAM_Claimed[_clSlot];
					gBAM_Claimed[_clSlot] = theValue;
					pthread_mutex_unlock(&gPlugIn_StateMutex);
					*outNumberPropertiesChanged = 1;
					outChangedAddresses[0] = *inAddress;
					if(_oldClaimed != theValue)
					{
						// Device list changed — notify HAL on plugin object.
						dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
							AudioObjectPropertyAddress _addr = { kAudioPlugInPropertyDeviceList, kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMain };
							gPlugIn_Host->PropertiesChanged(gPlugIn_Host, kObjectID_PlugIn, 1, &_addr);
						});
					}
				}
			}
		...
		case kBAMProperty_Channels:
			{
				FailWithAction(inDataSize != sizeof(CFPropertyListRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_SetDevicePropertyData: wrong size for kBAMProperty_Channels");
				CFNumberRef theNumber = *((const CFNumberRef*)inData);
				FailWithAction(theNumber == NULL || CFGetTypeID(theNumber) != CFNumberGetTypeID(), theAnswer = kAudioHardwareIllegalOperationError, Done, "BlackHole_SetDevicePropertyData: kBAMProperty_Channels not a CFNumber");
				UInt32 theValue = 0;
				CFNumberGetValue(theNumber, kCFNumberSInt32Type, &theValue);
				int _slot = BAM_DeviceIndexForObject(inObjectID);
				if (_slot >= 0) {
					pthread_mutex_lock(&gPlugIn_StateMutex);
					gBAM_Channels[_slot] = theValue;
					pthread_mutex_unlock(&gPlugIn_StateMutex);
				}
				*outNumberPropertiesChanged = 1;
				outChangedAddresses[0] = *inAddress;
		...
		case kBAMProperty_MixName:
			{
				FailWithAction(inDataSize != sizeof(CFStringRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_SetDevicePropertyData: wrong size for kBAMProperty_MixName");
				int _slot = BAM_DeviceIndexForObject(inObjectID);
				if (_slot >= 0) {
					pthread_mutex_lock(&gPlugIn_StateMutex);
					if(gBAM_MixName[_slot] != NULL) { CFRelease(gBAM_MixName[_slot]); }
					gBAM_MixName[_slot] = *((const CFStringRef*)inData);
					if(gBAM_MixName[_slot] != NULL) { CFRetain(gBAM_MixName[_slot]); }
					pthread_mutex_unlock(&gPlugIn_StateMutex);
				}
				*outNumberPropertiesChanged = 1;
				outChangedAddresses[0] = *inAddress;
...
static Boolean	BlackHole_HasStreamProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress)
{
	...
	FailIf(!BAM_IsStreamObject(inObjectID), Done, "BlackHole_HasStreamProperty: not a stream object");
...
static OSStatus	BlackHole_IsStreamPropertySettable(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, Boolean* outIsSettable)
{
	...
	FailWithAction(!BAM_IsStreamObject(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_IsStreamPropertySettable: not a stream object");
...
static OSStatus	BlackHole_GetStreamPropertyDataSize(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32* outDataSize)
{
	...
	FailWithAction(!BAM_IsStreamObject(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_GetStreamPropertyDataSize: not a stream object");
...
static OSStatus	BlackHole_GetStreamPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	FailWithAction(!BAM_IsStreamObject(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_GetStreamPropertyData: not a stream object");
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyOwner:
			...
			*((AudioObjectID*)outData) = BAM_DeviceID(BAM_DeviceIndexForObject(inObjectID));
			*outDataSize = sizeof(AudioObjectID);
			break;
		...
		case kAudioStreamPropertyIsActive:
			...
			pthread_mutex_lock(&gPlugIn_StateMutex);
			*((UInt32*)outData) = BAM_IsInputStream(inObjectID) ? gBAM_StreamInputActive[BAM_DeviceIndexForObject(inObjectID)] : gBAM_StreamOutputActive[BAM_DeviceIndexForObject(inObjectID)];
			pthread_mutex_unlock(&gPlugIn_StateMutex);
			*outDataSize = sizeof(UInt32);
			break;
		...
		case kAudioStreamPropertyDirection:
			...
			*((UInt32*)outData) = BAM_IsInputStream(inObjectID) ? 1 : 0;
			*outDataSize = sizeof(UInt32);
			break;
		...
		case kAudioStreamPropertyTerminalType:
			...
			*((UInt32*)outData) = BAM_IsInputStream(inObjectID) ? kAudioStreamTerminalTypeMicrophone : kAudioStreamTerminalTypeSpeaker;
			*outDataSize = sizeof(UInt32);
			break;
		...
		case kAudioStreamPropertyPhysicalFormat:
			...
			pthread_mutex_lock(&gPlugIn_StateMutex);
            ((AudioStreamBasicDescription*)outData)->mSampleRate = gBAM_SampleRate[BAM_DeviceIndexForObject(inObjectID)];
...
static OSStatus	BlackHole_SetStreamPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData, UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2])
{
	...
	FailWithAction(!BAM_IsStreamObject(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_SetStreamPropertyData: not a stream object");
	...
	*outNumberPropertiesChanged = 0;
	...
	switch(inAddress->mSelector)
	{
		case kAudioStreamPropertyIsActive:
			...
			{
				int _iaSlot = BAM_DeviceIndexForObject(inObjectID);
				bool _iaNew = (*((const UInt32*)inData) != 0);
				pthread_mutex_lock(&gPlugIn_StateMutex);
				bool* _iaFlag = BAM_IsInputStream(inObjectID) ? &gBAM_StreamInputActive[_iaSlot] : &gBAM_StreamOutputActive[_iaSlot];
				if(*_iaFlag != _iaNew)
				{
					*_iaFlag = _iaNew;
					*outNumberPropertiesChanged = 1;
		...
		case kAudioStreamPropertyPhysicalFormat:
			...
			int _strmSrSlot = BAM_DeviceIndexForObject(inObjectID);
			pthread_mutex_lock(&gPlugIn_StateMutex);
			theOldSampleRate = gBAM_SampleRate[_strmSrSlot];
			gBAM_RequestedSampleRate[_strmSrSlot] = ((const AudioStreamBasicDescription*)inData)->mSampleRate;
			pthread_mutex_unlock(&gPlugIn_StateMutex);
			if(((const AudioStreamBasicDescription*)inData)->mSampleRate != theOldSampleRate)
			{
				AudioObjectID _strmDevice = BAM_DeviceID(_strmSrSlot);
				dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{ gPlugIn_Host->RequestDeviceConfigurationChange(gPlugIn_Host, _strmDevice, ChangeAction_SetSampleRate, NULL); });
			}
			break;
...
// Control role classification by sub-object offset within a slot.
typedef enum { kCtrl_Volume, kCtrl_Mute, kCtrl_Pitch, kCtrl_Clock, kCtrl_Unknown } BAM_CtrlRole;

static BAM_CtrlRole BAM_ControlRole(AudioObjectID oid)
{
    if (oid < kObjectID_FirstDevice) return kCtrl_Unknown;
    int offset = (int)(oid - kObjectID_FirstDevice) % kObjectsPerDevice;
    int slot   = (int)(oid - kObjectID_FirstDevice) / kObjectsPerDevice;
    if (slot >= kBAM_MaxDevices) return kCtrl_Unknown;
    switch (offset) {
        case 2: return kCtrl_Volume; // VolumeInput
        case 3: return kCtrl_Mute;   // MuteInput
        case 5: return kCtrl_Volume; // VolumeOutput
        case 6: return kCtrl_Mute;   // MuteOutput (pitch on slot 0 uses same ID space)
        case 7: return kCtrl_Clock;
        default: return kCtrl_Unknown;
    }
}

// True if this control's scope is Input (offsets 2,3) vs Output (5,6,7).
static AudioObjectPropertyScope BAM_ControlScope(AudioObjectID oid)
{
    int offset = (int)(oid - kObjectID_FirstDevice) % kObjectsPerDevice;
    return (offset == 2 || offset == 3) ? kAudioObjectPropertyScopeInput : kAudioObjectPropertyScopeOutput;
}
...
static Boolean	BlackHole_HasControlProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress)
{
	...
	BAM_CtrlRole _role = BAM_ControlRole(inObjectID);
	switch (_role)
	{
		case kCtrl_Volume:
			switch(inAddress->mSelector)
	...
	switch (_role)
	{
		...
		case kCtrl_Pitch:
			switch(inAddress->mSelector)
			{
				...
				case kAudioControlPropertyScope:       *outDataSize = sizeof(AudioObjectPropertyScope); break;
				case kAudioControlPropertyElement:     *outDataSize = sizeof(AudioObjectPropertyElement); break;
				case kAudioStereoPanControlPropertyValue: *outDataSize = sizeof(Float32); break;
				default: theAnswer = kAudioHardwareUnknownPropertyError; break;
			};
			break;
		...
		case kCtrl_Clock:
			switch(inAddress->mSelector)
			{
				...
				case kAudioControlPropertyScope:       *outDataSize = sizeof(AudioObjectPropertyScope); break;
				case kAudioControlPropertyElement:     *outDataSize = sizeof(AudioObjectPropertyElement); break;
				case kAudioSelectorControlPropertyCurrentItem:    *outDataSize = sizeof(UInt32); break;
				case kAudioSelectorControlPropertyAvailableItems: *outDataSize = kClockSource_NumberItems * sizeof(UInt32); break;
				case kAudioSelectorControlPropertyItemName:       *outDataSize = sizeof(CFStringRef); break;
				default: theAnswer = kAudioHardwareUnknownPropertyError; break;
			};
			break;
...
static OSStatus	BlackHole_GetControlPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	BAM_CtrlRole _role = BAM_ControlRole(inObjectID);
	int _ctrlSlot = BAM_DeviceIndexForObject(inObjectID);
	AudioObjectID _ownerDevice = (_ctrlSlot >= 0) ? BAM_DeviceID(_ctrlSlot) : kObjectID_PlugIn;
	...
	switch (_role)
	{
		case kCtrl_Volume:
			...
			{
				case kAudioObjectPropertyBaseClass:
					FailWithAction(inDataSize < sizeof(AudioClassID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyBaseClass volume");
					*((AudioClassID*)outData) = kAudioLevelControlClassID; *outDataSize = sizeof(AudioClassID); break;
				case kAudioObjectPropertyClass:
					FailWithAction(inDataSize < sizeof(AudioClassID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyClass volume");
					*((AudioClassID*)outData) = kAudioVolumeControlClassID; *outDataSize = sizeof(AudioClassID); break;
				case kAudioObjectPropertyOwner:
					FailWithAction(inDataSize < sizeof(AudioObjectID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyOwner volume");
					*((AudioObjectID*)outData) = _ownerDevice; *outDataSize = sizeof(AudioObjectID); break;
				case kAudioObjectPropertyOwnedObjects:
					*outDataSize = 0; break;
				case kAudioControlPropertyScope:
					FailWithAction(inDataSize < sizeof(AudioObjectPropertyScope), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioControlPropertyScope volume");
					*((AudioObjectPropertyScope*)outData) = BAM_ControlScope(inObjectID); *outDataSize = sizeof(AudioObjectPropertyScope); break;
				case kAudioControlPropertyElement:
					FailWithAction(inDataSize < sizeof(AudioObjectPropertyElement), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioControlPropertyElement volume");
					*((AudioObjectPropertyElement*)outData) = kAudioObjectPropertyElementMain; *outDataSize = sizeof(AudioObjectPropertyElement); break;
				case kAudioLevelControlPropertyScalarValue:
					FailWithAction(inDataSize < sizeof(Float32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioLevelControlPropertyScalarValue");
					pthread_mutex_lock(&gPlugIn_StateMutex);
					*((Float32*)outData) = volume_to_scalar((_ctrlSlot >= 0) ? gBAM_Volume[_ctrlSlot] : 1.0f);
					pthread_mutex_unlock(&gPlugIn_StateMutex);
					*outDataSize = sizeof(Float32); break;
				case kAudioLevelControlPropertyDecibelValue:
					FailWithAction(inDataSize < sizeof(Float32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioLevelControlPropertyDecibelValue");
					pthread_mutex_lock(&gPlugIn_StateMutex);
					*((Float32*)outData) = volume_to_decibel((_ctrlSlot >= 0) ? gBAM_Volume[_ctrlSlot] : 1.0f);
					pthread_mutex_unlock(&gPlugIn_StateMutex);
					*outDataSize = sizeof(Float32); break;
				case kAudioLevelControlPropertyDecibelRange:
					FailWithAction(inDataSize < sizeof(AudioValueRange), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioLevelControlPropertyDecibelRange");
					((AudioValueRange*)outData)->mMinimum = kVolume_MinDB; ((AudioValueRange*)outData)->mMaximum = kVolume_MaxDB;
					*outDataSize = sizeof(AudioValueRange); break;
				case kAudioLevelControlPropertyConvertScalarToDecibels:
					FailWithAction(inDataSize < sizeof(Float32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for ConvertScalarToDecibels");
					...
					*outDataSize = sizeof(Float32); break;
				case kAudioLevelControlPropertyConvertDecibelsToScalar:
					FailWithAction(inDataSize < sizeof(Float32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for ConvertDecibelsToScalar");
					...
					*outDataSize = sizeof(Float32); break;
				default:
					theAnswer = kAudioHardwareUnknownPropertyError; break;
			};
			break;
		...
		case kCtrl_Mute:
			...
			{
				case kAudioObjectPropertyBaseClass:
					FailWithAction(inDataSize < sizeof(AudioClassID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyBaseClass mute");
					*((AudioClassID*)outData) = kAudioBooleanControlClassID; *outDataSize = sizeof(AudioClassID); break;
				case kAudioObjectPropertyClass:
					FailWithAction(inDataSize < sizeof(AudioClassID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyClass mute");
					*((AudioClassID*)outData) = kAudioMuteControlClassID; *outDataSize = sizeof(AudioClassID); break;
				case kAudioObjectPropertyOwner:
					FailWithAction(inDataSize < sizeof(AudioObjectID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyOwner mute");
					*((AudioObjectID*)outData) = _ownerDevice; *outDataSize = sizeof(AudioObjectID); break;
				case kAudioObjectPropertyOwnedObjects:
					*outDataSize = 0; break;
				case kAudioControlPropertyScope:
					FailWithAction(inDataSize < sizeof(AudioObjectPropertyScope), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioControlPropertyScope mute");
					*((AudioObjectPropertyScope*)outData) = BAM_ControlScope(inObjectID); *outDataSize = sizeof(AudioObjectPropertyScope); break;
				case kAudioControlPropertyElement:
					FailWithAction(inDataSize < sizeof(AudioObjectPropertyElement), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioControlPropertyElement mute");
					*((AudioObjectPropertyElement*)outData) = kAudioObjectPropertyElementMain; *outDataSize = sizeof(AudioObjectPropertyElement); break;
				case kAudioBooleanControlPropertyValue:
					FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioBooleanControlPropertyValue");
					pthread_mutex_lock(&gPlugIn_StateMutex);
					*((UInt32*)outData) = ((_ctrlSlot >= 0) ? gBAM_Mute[_ctrlSlot] : false) ? 1 : 0;
					pthread_mutex_unlock(&gPlugIn_StateMutex);
		...
		case kCtrl_Pitch:
			...
			{
				case kAudioObjectPropertyBaseClass:
					FailWithAction(inDataSize < sizeof(AudioClassID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyBaseClass pitch");
					*((AudioClassID*)outData) = kAudioStereoPanControlClassID; *outDataSize = sizeof(AudioClassID); break;
				case kAudioObjectPropertyClass:
					FailWithAction(inDataSize < sizeof(AudioClassID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyClass pitch");
					*((AudioClassID*)outData) = kAudioStereoPanControlClassID; *outDataSize = sizeof(AudioClassID); break;
				case kAudioObjectPropertyOwner:
					FailWithAction(inDataSize < sizeof(AudioObjectID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyOwner pitch");
					*((AudioObjectID*)outData) = _ownerDevice; *outDataSize = sizeof(AudioObjectID); break;
				case kAudioObjectPropertyOwnedObjects:
					*outDataSize = 0; break;
				case kAudioControlPropertyScope:
					FailWithAction(inDataSize < sizeof(AudioObjectPropertyScope), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioControlPropertyScope pitch");
					*((AudioObjectPropertyScope*)outData) = kAudioObjectPropertyScopeOutput; *outDataSize = sizeof(AudioObjectPropertyScope); break;
				case kAudioControlPropertyElement:
					FailWithAction(inDataSize < sizeof(AudioObjectPropertyElement), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioControlPropertyElement pitch");
					*((AudioObjectPropertyElement*)outData) = kAudioObjectPropertyElementMain; *outDataSize = sizeof(AudioObjectPropertyElement); break;
				case kAudioStereoPanControlPropertyValue:
					FailWithAction(inDataSize < sizeof(Float32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioStereoPanControlPropertyValue");
					pthread_mutex_lock(&gPlugIn_StateMutex);
					*((Float32*)outData) = (_ctrlSlot >= 0) ? gBAM_PitchAdjust[_ctrlSlot] : 0.5f;
					pthread_mutex_unlock(&gPlugIn_StateMutex);
					*outDataSize = sizeof(Float32); break;
				default:
					theAnswer = kAudioHardwareUnknownPropertyError; break;
		...
		case kCtrl_Clock:
			...
			{
				case kAudioObjectPropertyBaseClass:
					FailWithAction(inDataSize < sizeof(AudioClassID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyBaseClass clock");
					*((AudioClassID*)outData) = kAudioSelectorControlClassID; *outDataSize = sizeof(AudioClassID); break;
				case kAudioObjectPropertyClass:
					FailWithAction(inDataSize < sizeof(AudioClassID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyClass clock");
					*((AudioClassID*)outData) = kAudioClockSourceControlClassID; *outDataSize = sizeof(AudioClassID); break;
				case kAudioObjectPropertyOwner:
					FailWithAction(inDataSize < sizeof(AudioObjectID), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioObjectPropertyOwner clock");
					*((AudioObjectID*)outData) = _ownerDevice; *outDataSize = sizeof(AudioObjectID); break;
				case kAudioObjectPropertyOwnedObjects:
					*outDataSize = 0; break;
				case kAudioControlPropertyScope:
					FailWithAction(inDataSize < sizeof(AudioObjectPropertyScope), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioControlPropertyScope clock");
					*((AudioObjectPropertyScope*)outData) = kAudioObjectPropertyScopeGlobal; *outDataSize = sizeof(AudioObjectPropertyScope); break;
				case kAudioControlPropertyElement:
					FailWithAction(inDataSize < sizeof(AudioObjectPropertyElement), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioControlPropertyElement clock");
					*((AudioObjectPropertyElement*)outData) = kAudioObjectPropertyElementMain; *outDataSize = sizeof(AudioObjectPropertyElement); break;
				case kAudioSelectorControlPropertyCurrentItem:
					FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioSelectorControlPropertyCurrentItem");
					pthread_mutex_lock(&gPlugIn_StateMutex);
					*((UInt32*)outData) = (_ctrlSlot >= 0) ? gBAM_ClockSource[_ctrlSlot] : 0;
					pthread_mutex_unlock(&gPlugIn_StateMutex);
					*outDataSize = sizeof(UInt32); break;
				case kAudioSelectorControlPropertyAvailableItems:
					theNumberItemsToFetch = inDataSize / sizeof(UInt32);
					if(theNumberItemsToFetch > kClockSource_NumberItems) theNumberItemsToFetch = kClockSource_NumberItems;
					for(theItemIndex = 0; theItemIndex < theNumberItemsToFetch; ++theItemIndex) ((UInt32*)outData)[theItemIndex] = theItemIndex;
					*outDataSize = theNumberItemsToFetch * sizeof(UInt32); break;
				case kAudioSelectorControlPropertyItemName:
					FailWithAction(inDataSize < sizeof(CFStringRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: not enough space for kAudioSelectorControlPropertyItemName");
					FailWithAction(inQualifierDataSize != sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError, Done, "BlackHole_GetControlPropertyData: wrong qualifier size for kAudioSelectorControlPropertyItemName");
					FailWithAction(*((const UInt32*)inQualifierData) >= kClockSource_NumberItems, theAnswer = kAudioHardwareIllegalOperationError, Done, "BlackHole_GetControlPropertyData: item out of range for kAudioSelectorControlPropertyItemName");
					*(CFStringRef*)outData = (*(UInt32*)inQualifierData == 0) ? CFSTR(kClockSource_InternalFixed) : CFSTR(kClockSource_InternalAdjustable);
...
static OSStatus	BlackHole_SetControlPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData, UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2])
{
	...
	*outNumberPropertiesChanged = 0;
	...
	//	Dispatch by control role derived from the object ID offset.
	int _setCtrlSlot = BAM_DeviceIndexForObject(inObjectID);
	if(_setCtrlSlot < 0)
	{
		theAnswer = kAudioHardwareBadObjectError;
		goto Done;
	}
	BAM_CtrlRole _setRole = BAM_ControlRole(inObjectID);
	if(_setRole == kCtrl_Volume)
	{
		switch(inAddress->mSelector)
		{
			case kAudioLevelControlPropertyScalarValue:
				...
				if(theNewVolume < 0.0) theNewVolume = 0.0;
				else if(theNewVolume > 1.0) theNewVolume = 1.0;
				pthread_mutex_lock(&gPlugIn_StateMutex);
				if(gBAM_Volume[_setCtrlSlot] != theNewVolume)
				{
					gBAM_Volume[_setCtrlSlot] = theNewVolume;
					*outNumberPropertiesChanged = 2;
			...
			case kAudioLevelControlPropertyDecibelValue:
				...
				else if(theNewVolume > kVolume_MaxDB) theNewVolume = kVolume_MaxDB;
	...
	else if(_setRole == kCtrl_Mute)
	{
		switch(inAddress->mSelector)
		{
			case kAudioBooleanControlPropertyValue:
				...
				pthread_mutex_lock(&gPlugIn_StateMutex);
				if(gBAM_Mute[_setCtrlSlot] != (*((const UInt32*)inData) != 0))
				{
					gBAM_Mute[_setCtrlSlot] = *((const UInt32*)inData) != 0;
					*outNumberPropertiesChanged = 1;
	...
	else if(_setRole == kCtrl_Pitch)
	{
		switch(inAddress->mSelector)
		{
			case kAudioStereoPanControlPropertyValue:
				...
				if(theNewPitch < 0.0) theNewPitch = 0.0;
				else if(theNewPitch > 1.0) theNewPitch = 1.0;
				pthread_mutex_lock(&gPlugIn_StateMutex);
				if(gBAM_PitchAdjust[_setCtrlSlot] != theNewPitch)
				{
					gBAM_PitchAdjust[_setCtrlSlot] = theNewPitch;
					gBAM_AdjustedTicksPerFrame[_setCtrlSlot] = gBAM_HostTicksPerFrame[_setCtrlSlot] - gBAM_HostTicksPerFrame[_setCtrlSlot]/100.0 * 2.0*(theNewPitch - 0.5);
					*outNumberPropertiesChanged = 1;
	...
	else if(_setRole == kCtrl_Clock)
	{
		switch(inAddress->mSelector)
		{
			case kAudioSelectorControlPropertyCurrentItem:
				...
				if(theNewSource >= kClockSource_NumberItems) theNewSource = kClockSource_NumberItems - 1;
				pthread_mutex_lock(&gPlugIn_StateMutex);
				if(gBAM_ClockSource[_setCtrlSlot] != theNewSource)
				{
					gBAM_ClockSource[_setCtrlSlot] = theNewSource;
					UInt64 changeAction = (theNewSource > 0) ? ChangeAction_EnablePitchControl : ChangeAction_DisablePitchControl;
					AudioObjectID _clockDevice = BAM_DeviceID(_setCtrlSlot);
					*outNumberPropertiesChanged = 1;
					outChangedAddresses[0].mSelector = kAudioSelectorControlPropertyCurrentItem;
					outChangedAddresses[0].mScope = kAudioObjectPropertyScopeGlobal;
					outChangedAddresses[0].mElement = kAudioObjectPropertyElementMain;
					dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
						gPlugIn_Host->RequestDeviceConfigurationChange(gPlugIn_Host, _clockDevice, changeAction, NULL);
					});
				}
				pthread_mutex_unlock(&gPlugIn_StateMutex);
				break;
...
static OSStatus	BlackHole_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
	...
	int _startSlot = BAM_DeviceIndexForObject(inDeviceObjectID);
	FailWithAction(_startSlot < 0 || !BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_StartIO: bad device ID");
	FailWithAction(gBAM_IOIsRunning[_startSlot] == UINT64_MAX, theAnswer = kAudioHardwareIllegalOperationError, Done, "BlackHole_StartIO: overflow error.");

	//	we need to hold the state lock
	pthread_mutex_lock(&gPlugIn_StateMutex);

	gBAM_IOIsRunning[_startSlot] += 1;

	// allocate per-slot ring buffer on first IO start for this slot
	if(gBAM_IOIsRunning[_startSlot] == 1 && gBAM_RingBuffer[_startSlot] == NULL)
	{
		gBAM_NumberTimeStamps[_startSlot] = 0;
		gBAM_AnchorSampleTime[_startSlot] = 0;
		gBAM_AnchorHostTime[_startSlot] = mach_absolute_time();
		gBAM_PreviousTicks[_startSlot] = 0;
		gBAM_RingBuffer[_startSlot] = calloc(kRing_Buffer_Frame_Size * kNumber_Of_Channels, sizeof(Float32));
	}
...
static OSStatus	BlackHole_StopIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
	...
	int _stopSlot = BAM_DeviceIndexForObject(inDeviceObjectID);
	FailWithAction(_stopSlot < 0 || !BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_StopIO: bad device ID");
	FailWithAction(gBAM_IOIsRunning[_stopSlot] == 0, theAnswer = kAudioHardwareIllegalOperationError, Done, "BlackHole_StopIO: underflow error.");

	//	we need to hold the state lock
	pthread_mutex_lock(&gPlugIn_StateMutex);

	gBAM_IOIsRunning[_stopSlot] -= 1;

	// free per-slot ring buffer when last client stops
	if(gBAM_IOIsRunning[_stopSlot] == 0 && gBAM_RingBuffer[_stopSlot] != NULL)
	{
		free(gBAM_RingBuffer[_stopSlot]);
		gBAM_RingBuffer[_stopSlot] = NULL;
	}
...
static OSStatus	BlackHole_GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, Float64* outSampleTime, UInt64* outHostTime, UInt64* outSeed)
{
	...
	int _tsSlot = BAM_DeviceIndexForObject(inDeviceObjectID);
	FailWithAction(_tsSlot < 0 || !BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_GetZeroTimeStamp: bad device ID");
	...
	theHostTicksPerRingBuffer = gBAM_HostTicksPerFrame[_tsSlot] * ((Float64)kDevice_RingBufferSize);
	if(gBAM_ClockSource[_tsSlot] > 0)
	{
		theAdjustedTicksPerRingBuffer = gBAM_AdjustedTicksPerFrame[_tsSlot] * ((Float64)kDevice_RingBufferSize);
	}
	else
	{
		theAdjustedTicksPerRingBuffer = gBAM_HostTicksPerFrame[_tsSlot] * ((Float64)kDevice_RingBufferSize);
	}

	theNextTickOffset = gBAM_PreviousTicks[_tsSlot] + theAdjustedTicksPerRingBuffer;
	theNextHostTime = gBAM_AnchorHostTime[_tsSlot] + ((UInt64)theNextTickOffset);

	//	go to the next time if the next host time is less than the current time
	if(theNextHostTime <= theCurrentHostTime)
	{
		++gBAM_NumberTimeStamps[_tsSlot];
		gBAM_PreviousTicks[_tsSlot] = theNextTickOffset;
	}

	//	set the return values
	*outSampleTime = gBAM_NumberTimeStamps[_tsSlot] * kDevice_RingBufferSize;
	*outHostTime = gBAM_AnchorHostTime[_tsSlot] + gBAM_PreviousTicks[_tsSlot];
	*outSeed = 1;
...
static OSStatus	BlackHole_WillDoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, Boolean* outWillDo, Boolean* outWillDoInPlace)
{
	...
	FailWithAction(!BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_WillDoIOOperation: bad device ID");
...
static OSStatus	BlackHole_BeginIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo)
{
	...
	FailWithAction(!BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_BeginIOOperation: bad device ID");
...
static OSStatus	BlackHole_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
	...
	int _ioSlot = BAM_DeviceIndexForObject(inDeviceObjectID);
	FailWithAction(_ioSlot < 0 || !BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_DoIOOperation: bad device ID");
	FailWithAction(!BAM_IsStreamObject(inStreamObjectID) || BAM_DeviceIndexForObject(inStreamObjectID) != _ioSlot, theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_DoIOOperation: bad stream ID");

	Float32* _ringBuf = gBAM_RingBuffer[_ioSlot];
    ...
    // Keep track of last outputSampleTime and cleared buffer status per slot.
    static Float64 lastOutputSampleTime[kBAM_MaxDevices] = {0};
    static Boolean isBufferClear[kBAM_MaxDevices];

    // From BlackHole to Application
    if(inOperationID == kAudioServerPlugInIOOperationReadInput)
    {
        if(gBAM_Mute[_ioSlot] || lastOutputSampleTime[_ioSlot] - inIOBufferFrameSize < inIOCycleInfo->mInputTime.mSampleTime)
        {
            vDSP_vclr(ioMainBuffer, 1, inIOBufferFrameSize * kNumber_Of_Channels);

            if(!isBufferClear[_ioSlot])
            {
                vDSP_vclr(_ringBuf, 1, kRing_Buffer_Frame_Size * kNumber_Of_Channels);
                isBufferClear[_ioSlot] = true;
            }
        }
        else
        {
            memcpy(ioMainBuffer, _ringBuf + ringBufferFrameLocationStart * kNumber_Of_Channels, firstPartFrameSize * kNumber_Of_Channels * sizeof(Float32));
            memcpy((Float32*)ioMainBuffer + firstPartFrameSize * kNumber_Of_Channels, _ringBuf, secondPartFrameSize * kNumber_Of_Channels * sizeof(Float32));

            if(kEnableVolumeControl)
            {
                vDSP_vsmul(ioMainBuffer, 1, &gBAM_Volume[_ioSlot], ioMainBuffer, 1, inIOBufferFrameSize * kNumber_Of_Channels);
            }
        }
    }
    ...
    if(inOperationID == kAudioServerPlugInIOOperationWriteMix)
    {
        ...
        memcpy(_ringBuf + ringBufferFrameLocationStart * kNumber_Of_Channels, ioMainBuffer, firstPartFrameSize * kNumber_Of_Channels * sizeof(Float32));
        memcpy(_ringBuf, (Float32*)ioMainBuffer + firstPartFrameSize * kNumber_Of_Channels, secondPartFrameSize * kNumber_Of_Channels * sizeof(Float32));

        lastOutputSampleTime[_ioSlot] = inIOCycleInfo->mOutputTime.mSampleTime + inIOBufferFrameSize;
        isBufferClear[_ioSlot] = false;
    }
...
static OSStatus	BlackHole_EndIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo)
{
	...
	FailWithAction(!BAM_IsDeviceObject(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_EndIOOperation: bad device ID");
